...
class App extends Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener('resize', () => {
      ...
      data.graphics.resolution = `${window.screen.width} x ${window.screen.height}`
    ...
    window.addEventListener('textcopied', (e) => {
      ...
      if (e.detail) {
        console.log(e.detail)
        this.addToast(`Copied ${e.detail.name} "${e.detail.value}" to clip board.`)
      }
    })
  }
    ...
    return (
      <div className="App">
        ...
        <ul className="App-toasts">
            {this.state.toasts.map(toast => <li className="App-toast" key={toast.id}>{toast.text}</li>)}
        </ul>
      </div>
    );
  }
